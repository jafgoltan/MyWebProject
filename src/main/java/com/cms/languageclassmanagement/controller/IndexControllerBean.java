...
import com.cms.languageclassmanagement.model.*;
import com.cms.languageclassmanagement.service.*;
...
public class IndexControllerBean {
    ...
    public void buy() {
      ...
      Period  period=periodService.findPeriodById(Integer.valueOf(id));
     ...
     if(period.getMaxstudent() != period.getUsers().size()) {
         ...
         if (checkDate(user, period)) {
             ...
             Set<Period> periods = user.getPeriods();
             periods.add(period);
             user.setPeriods(periods);
...
public boolean checkDate(User user, Period period) {
...
int day = calendaru.get(Calendar.DAY_OF_YEAR);
  ...
  //  int diff2= day + DiffrentTwoDay(period.getSeconddayinweek()) ;
    ...
    for(Period per :user.getPeriods()) {
